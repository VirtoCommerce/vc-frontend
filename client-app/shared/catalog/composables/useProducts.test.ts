...
describe("useProducts", () => {
  ...
  // Chips, the "Reset filters" button and the facet expression all read the prepared filters, so a
  // filter the user cannot remove (generated by the server, or carried in its own route param) must
  // never reach them.
  describe("prepared filters", () => {
    // The name is deliberately outside EXCLUDED_FILTER_NAMES, so only the isGenerated flag can drop it.
    it("drops server-generated filters and keeps the user's own", () => {
      ...
      expect(productsFilters.value.filters.map((filter) => filter.name)).toEqual(["color"]);
