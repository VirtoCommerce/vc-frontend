  ...
  <div>
    ...
    <div class="flex flex-col">
      ...
      <div ref="listElement" class="mt-5 w-full">
        <template v-if="subscriptionsItems.length">
          <VcWidget size="lg">
            <template v-if="allLoading">
              ...
              <div v-else class="flex flex-col rounded border bg-additional-50 shadow-sm">
                <ProductSkeletonList v-for="index in actualPageRowsCount" :key="index" class="even:bg-neutral-50" />
              </div>
            </template>
...
import { InStock, CountInCart, ProductSkeletonGrid, useProducts, ProductSkeletonList } from "@/shared/catalog";
...
const keyword = ref(fetchParameters.value.keyword);
...
async function applyKeyword(): Promise<void> {
  fetchParameters.value.keyword = keyword.value;
