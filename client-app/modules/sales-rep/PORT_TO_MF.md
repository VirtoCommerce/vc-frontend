...
| Vc components used in templates but never imported | The module's templates reach for **21**; the facade exports **7** (`VcWidget VcButton VcInput VcCheckbox VcMarkdownRender VcModal VcWidgetSkeleton`). Missing **15**: `VcAlert VcBadge VcBreadcrumbs VcEmptyView VcIcon VcImage VcLabel VcLink VcLoaderOverlay VcMenuItem VcSelect VcTable VcTableColumn VcTextarea VcTypography`. Whether a remote's templates resolve the host's global registrations depends on the remote rendering inside the host's app instance — and the standalone dev server has no host registrations at all, so they are needed there regardless. **Settle this before the facade release** (same reason as the test utilities: the pin is a tarball URL, so adding them later re-pins every plugin) |
