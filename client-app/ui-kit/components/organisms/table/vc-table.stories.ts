...
export const SelectionCustomHeader: StoryType = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "A custom `#header` replaces the built-in header row, selection cell included, while body rows keep theirs — so the header has to render its own, spreading `selectionColumnAttrs` on the `<th>` for the matching width and sticky behavior. Spread `headAttrs` on the `<thead>` too: it carries the sticky modifier, without which `sticky-header` and `max-height` have no effect on a custom header — this story sets `max-height`, so scroll the body to see the header hold. Add your own classes with a sibling `class` attribute rather than editing `headAttrs.class`. Guard the select-all control with `selectionMode === 'multiple'`, as the built-in header does — switch the `selectionMode` control to `single` and the cell keeps an `sr-only` label instead of going empty. Omit the cell entirely and every column shifts by one; in DEV, VcTable warns about it in the console. The scope carries no per-column attributes, so over `fixed` columns a custom header has to reproduce the sticky offsets itself — the built-in header cells are the only ones that get them. The labels below are plain strings for the demo: in app code translate them, the way the built-in header uses `ui_kit.table.select_all` / `ui_kit.table.deselect_all` / `ui_kit.table.selection_column`.",
      },
      source: {
        code: `
