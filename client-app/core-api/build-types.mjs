 ...
 */
function autoBumpMinorIfContractChanged(currentContract, currentPreset, currentVersion) {
  ...
  if (decision.action === "bump-minor") {
    ...
    step(
      `contract or tailwind preset changed vs ${base.baseRef} — version auto-bumped ${current} -> ${next} (commit package.json too).`,
    );
  }
}
...
if (CHECK_MODE) {
    ...
    if (decision.action === "bump-minor") {
      fail(
        `the public contract or tailwind preset changed relative to ${base.baseRef}, but CORE_VERSION is still ` +
          `${base.baseVersion}. Run \`yarn build:core-types\` (auto-bumps minor).`,
      );
    }
  }
  if (base) {
  ...
  autoBumpMinorIfContractChanged(contract, tailwindPreset, corePkg.version);
