...
const PRESET_OUT_FILE = resolve(DIST_DIR, "tailwind-preset.cjs");
...
function compareContractToBase(currentContract, currentPreset) {
  ...
  const basePreset = baseSha ? git(["show", `${baseSha}:client-app/core-api/dist/tailwind-preset.cjs`]) : { status: 1 };
...
if (CHECK_MODE) {
  ...
  if (committedPreset !== tailwindPreset) {
    fail("committed dist/tailwind-preset.cjs is stale — run `yarn build:core-types` and commit the result.");
  }
