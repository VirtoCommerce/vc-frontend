...
export function getMainRoutes(supportedLocales: string[]): RouteRecordRaw[] {
  const supportedLocalesRegexPattern = supportedLocales.length ? supportedLocales.join("|") : "NEVERMATCH";

  return [
    {
      path: `/:locale(${supportedLocalesRegexPattern})?`,
      children: [
        {
          path: "auth/callback",
          name: "AuthCallback",
          component: callback,
          meta: { public: true, redirectable: false },
        },
